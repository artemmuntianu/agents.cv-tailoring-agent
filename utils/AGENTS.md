...
| `db.py` | Job store + claim semantics + schema DDL. `SCHEMA_SQL` is the **only** DDL and also creates the backoffice's tables - `resume_board` (with its archive columns), `resume_history` (four kinds, `Candidate`/`Company` actors), `board_actions` and `app_users` (the backoffice shares this database). Guarded `do $ddl$` blocks upgrade an existing database in place (job-id shape, archive shape, the actor rename, the kind list) | `get_db`, `job_key`, `new_job_id`, `JOB_FIELDS`, `SCHEMA_SQL`, `ACTIVE_STATUSES` |
